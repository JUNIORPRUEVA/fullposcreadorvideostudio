...
import { verifyMediaSignature } from "./lib/media-signature.js";
...
app.enableCors({
  ...
  // PUT: necesario para guardar la "Voz FullPOS" (PUT /voice/voice-preference) desde el navegador.
...
app.use((request: Request, response: Response, next: NextFunction) => {
  ...
  // Las URLs firmadas permiten que un <video> pida el archivo sin cabeceras.
  ...
  if (secret && signature && verifyMediaSignature(path, signature, expires, secret)) return next();
